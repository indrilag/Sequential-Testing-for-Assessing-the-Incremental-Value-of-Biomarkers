...
out <- function(N, mu_case1, mu_case2, mu_control, sigma, gamma, delta, rho, V1, V2){
    ...
    if((is.na(test_stat)==FALSE) && (test_stat < 0)){ #Replace 0 by 0.76 for Pocock boundary
      C= -1
    } else if((is.na(test_stat)==FALSE) && (test_stat> 2.36)){ #Replace 2.36 by 1.83 for Pocock boundary
      C=0
    } else{C=1}
    C2=0
    if(C==1){
      ...
      if((is.na(test_stat2)==FALSE) && (test_stat2 < 1.67)){C2=-1} #Replace 1.67 by 1.83 for Pocock boundary
      else{C2=2}
    }
    if(C==1 ){
      V1 <- V1-1
